...
### Draft Update Rule
`DraftUpdateWorker` must NOT update B-S prices when US market is closed. Market hours: Mon-Fri, 4:00 AM – 8:00 PM Eastern Time (pre-market through after-hours).

### LTR Display Rule
ALL money amounts with +/- signs MUST use LTR wrapper:
```kotlin
CompositionLocalProvider(LocalLayoutDirection provides LayoutDirection.Ltr) { ... }
```
ALL dates, ALL English content also LTR. NEVER show +/- on the right side of a number. This rule has been violated multiple times (see `state.md` "Known patterns broken multiple times").

### Build Rule (mandatory pre-commit)
```
...
**Symptom:** Some screens look correct in light mode but wrong in dark (or vice versa).
**Reason:** Direct `Color(0xFF...)` calls instead of `AppTheme.colors.X`.
**Fix:** Grep `Color(0x` outside the theme module. Replace with semantic tokens. Background agent has done this twice already (29 replacements across 7 files in April).

### RTL: use `start`/`end`, not `left`/`right`
**Symptom:** Hebrew RTL UI looks wrong in some screens (margins on wrong side).
**Reason:** `padding(start=)`/`(end=)` flip with locale; `(left=)`/`(right=)` don't.
**Fix:** Always use `start`/`end`.

### LTR for numbers + dates (CLAUDE.md rule)
**Symptom:** "−329" renders as "329−" because RTL locale flips the sign to the right.
**Fix:** Wrap every money/date/English-content composable in `CompositionLocalProvider(LocalLayoutDirection provides LayoutDirection.Ltr)`. **This rule has been broken in 6+ screens repeatedly.**
...
**Fix:** Decouple visual direction from logical action — track open issues S2 in roadmap.
