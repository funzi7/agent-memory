...
Calendar layout stability (GL9, hash d9c94fd): the calendar JUMPED on month nav from THREE variable-height sources, all in CalendarScreen.kt. (1) CalendarGrid rows = (totalCells+6)/7 = 5 OR 6 → force val rows = 6 always. (2) the out-of-range trailing cell was Spacer(Modifier.weight(1f)) which has ZERO height, so a fully-empty 6th row added nothing — must be Box(Modifier.weight(1f).aspectRatio(0.85f)) (same as a real day cell) to reserve a full row. (3) the per-week "שבוע: $X" total row and the GL8 stock-realized/combined header lines were conditional if-blocks → reserve their height by ALWAYS rendering them and toggling Modifier.alpha(if (cond) 1f else 0f) (no hardcoded dp, font-scale-robust). Header title-row jump: a go-to-today TextButton placed BELOW the month name grows the center Column past the ~48dp IconButton row only when shown → put the chip INLINE in a Row beside the name so the row height is constant. RTL note: in a Row, the 2nd child renders to the visual LEFT of the 1st, so Row { monthName; todayChip } puts the chip on the name's left. Left the progress bar (monthlyTarget>0) conditional — targets are consistent across the recently-navigated months.

### 2026-06-21 — PUT risk and intel must share one volatility-buffer assessment
- OPT: 24c50b6
- Never infer "far" from raw assignment probability alone: a short DTE can drive probability below 10% while the strike remains only ~4% OTM. Compute bufferPct=(price-strike)/price*100 and expectedMovePct=effectiveIV*sqrt(max(DTE,1)/365)*100; VERY_LOW requires at least 8% buffer and 1.5 expected moves after probability thresholds. For short PUTs, buffer<8%, MEDIUM+, or ITM must also cap StrategicRiskAnalyzer below the green threshold so high-IV premium bonuses cannot contradict the risk card. Keep this heuristic display-only; do not route it into ProfitCalculator/P&L.

### 2026-06-21 — Reports must use persisted stock month keys verbatim
- OPT: 65014e2
- stockRealizedByTicker month keys are stored as "YYYY-MM" strings. For Reports, consume the proven DashboardViewModel.stockRealized flow (as Calendar does), resolve a selected month with YearMonth.toString(), and resolve a selected year by the "<year>-" prefix. Keep the reports VM's read-only flow eager/period-resolved, and use REPORTS_STOCK to verify period/options/stock/combined without logging raw preferences.

Calendar summary-card height STILL varied after GL9's alpha fix (GL10, hash 9da346f): alpha keeps a row in layout but does NOT stop its TEXT from wrapping — a large "סה"כ החודש (כולל מניות): +$38,598.00" wrapped to 2 lines on stock months while the invisible "$0.00" placeholder stayed 1 line, so the card grew. Always add maxLines=1, softWrap=false to alpha-reserved lines whose value length varies, or the reserved height is not actually constant. Month-title reflow (GL10 TASK B): a variable-length month name in a Row reflows the surrounding arrows/chip. Pin them by putting the name in a FIXED-WIDTH centered Box (Modifier.width(140.dp) fits the longest Hebrew month+year at titleLarge, maxLines=1) and RESERVING the "היום" chip slot always (enabled=!isCurrent + alpha(if isCurrent 0f else 1f), NOT an if-block) so the center group is constant width; SpaceBetween then centers that constant-width group between the fixed arrows → nothing around the name moves. alpha reserves WIDTH the same way it reserves height (layout still runs; only drawing is skipped).

### 2026-06-29 Covered Put accounting pitfalls
- **Short-share NETTING on assignment.** A Covered Put put assignment BUYS shares at the strike that REDUCE the short (buy-to-cover). It must NEVER open a new long that stays open. `CoveredPutCalculator.assign` clamps covered shares to the short (`buyToCoverShares.min(shortShares)`) so an oversized assignment can't flip the position long. Short stock lives in the IBKR snapshot JSON (not the positions table) — the option entity only models the economics.
- **Premium double-counting.** On a CSP assignment the premium FOLDS into the acquired long shares' cost basis → option realizes $0 (existing behavior, unchanged). On a COVERED_PUT assignment it does NOT fold — the buy-to-cover is at the raw strike and the option premium is realized as income. ProfitCalculator.realizedPnL handles COVERED_PUT in the ASSIGNED branch (realize totalPremium) and EXCLUDES it from `isAssignedCspNoSale` zero-out. Do NOT also add the short-stock realized gain on the option side — that gain is tracked separately via the snapshot/StockRealized, so adding it here would double-count.
- **Partial assignment.** Only the assigned slice realizes: shortRealized = (entry-strike)*coveredByThisSlice, premiumRealized = perContractPremium*assignedContracts. Remaining contracts stay OPEN, remaining short = short - coveredByThisSlice. 5 of 13 MULL puts → 500 covered, 850 short remain, 8 contracts open.
- **Coverage is directional and must not be silently relabeled.** Two ways to become uncovered: (a) puts bigger than the short at open (oversized → `uncoveredPutShares > 0`, warn, don't label "fully covered"); (b) the user independently buys to cover the short before expiration, shrinking it below the put-covered shares (`recheckCoverageAfterShortReduction`). Covering 1300 of 1350 MULL short → 50 short vs 1300 put shares → 1250 now uncovered.
- **Split-adjusted contracts.** A Covered Put opened AFTER a forward split uses ordinary 100-share contracts — only the SHORT stock is split-adjusted (54@714.78 -25:1-> 1350@28.5912, economic value 38598.12 invariant). Do NOT assume every option after a split is standard: a PRE-split option adjusts strike/ratio (or deliverable/multiplier for odd splits) — `applyForwardSplitToOption` handles whole-number forward splits (strike/ratio, count*ratio, multiplier unchanged).
- **Prices keep 4 decimals; only money totals round to cents.** `effectiveCoverPrice`/`upsideBreakEven` are per-share PRICES (28.5912+1.40 = 29.9912) and must NOT be cent-rounded — early version rounded upsideBreakEven to 29.99 and the test caught it. Only money() (premium, P/L totals) rounds to 2dp HALF_UP.

### 2026-09-06 S1-final — phone install-loop lessons (Claude Code; OPT 7225b7af16c183de00a9f064ead03a01ad6af1d3)
- **Debug-keystore continuity = ONE file.** The build has no custom signingConfig, so `/root/.android/debug.keystore` is the entire debug signing path. The only identity that can update the installed app is the computer's ORIGINAL keystore (SHA-1 `5D:3D:85:5C:6C:6C:39:7F:81:7D:F2:BD:0C:62:F1:6F:94:0B:15:51`, created 2026-02-22); it now lives on the phone (the 2026-07-22 phone-generated one is retired as `debug.keystore.phone-generated.bak` — never restore it). Before EVERY `adb install -r`: `keytool` on the keystore + `apksigner --print-certs` on the new APK + on the freshly PULLED installed base.apk — all three fingerprints must be identical. Never "fix" a mismatch by uninstalling (wipes the Room DB / DataStore).
- **Gradle `UP-TO-DATE` on `:app:testDebugUnitTest` means the tests did NOT run.** With unchanged inputs Gradle re-uses the old result; for real evidence run `:app:cleanTestDebugUnitTest :app:testDebugUnitTest` and read `tests=` / `failures=` / `errors=` from `app/build/test-results/testDebugUnitTest/*.xml`. Same logic for the compile gate: `UP-TO-DATE` is only a valid pass because the inputs are byte-identical to the last real compile.
- **`adb shell date '+%m-%d %H:%M:%S.000'` loses its quotes.** `adb shell` re-splits the argument on the device ("date: Max 1 argument"), the `logcat -T` marker comes back EMPTY and the "bounded" dump silently holds ~3 lines. Wrap the whole remote command in double quotes: `adb shell "date '+%m-%d %H:%M:%S.000'"`. Fallback that always works: `adb logcat -d --pid=$(adb shell pidof -s com.dima.optionstracker)`.
- **"AndroidRuntime" in a full logcat dump is not necessarily a crash.** The `monkey` launcher itself logs `AndroidRuntime: >>>>>> START com.android.internal.os.RuntimeInit uid 2000 … VM exiting with result code 0`. Grep for `FATAL EXCEPTION` and restrict to the app PID before calling anything a crash.
- **Dashboard load emits ~100k logcat lines** (`PNL_TRACE` ≈96.8k + `PnLDebug` + `PNL_DBG` within ~10 s on this data set). It floods the ring buffer (real errors get harder to find) and prints per-position trade details. Diagnostic-only; the gating/removal TODO is in roadmap.md.
- **Another app taking the foreground is not an app failure.** Samsung `Pageboost: moveTaskToFront` lines mark a Recents-style switch by the person holding the phone. Verify OUR activity with `Displayed …MainActivity` + `scheduleTopResumedActivityChanged onTop=true` + a stable PID, not with `topResumedActivity` sampled at a random later instant.
- **`agent-memory-finalize` needs a CWD whose git toplevel basename is `options-profit-tracker`.** The script derives the agent-memory sub-directory from `basename $(git rev-parse --show-toplevel)`; run from the OPT clone (`/root/work/OptionsProfitTracker`) it looks for `agent-memory/OptionsProfitTracker` and exits 66. Working invocation (S1-final): `mkdir -p <scratch>/options-profit-tracker && git -C <scratch>/options-profit-tracker init -q && (cd <scratch>/options-profit-tracker && /root/work/bin/agent-memory-finalize "<msg>")` — the throwaway repo only supplies the name; staging (`options-profit-tracker/` only), the `--only` commit, and the push still happen inside the finalizer under its lock.

### 2026-09-07 S2 lessons (Claude Code; OPT f327a7e, PR #19)
- **"One-time" startup passes that are not flagged run on EVERY cold start.** The FT2 feed reconciliation in OptionsTrackerApp had no done-flag, so it rewrote every close row's timestamp to 16:00 ET on each launch — that alone produced the owner's "15.08 03:00" feed rows (16:00 ET = 03:00 next day in Thailand). Any startup "migration" must either carry a flag or be idempotent AND never clobber a real value.
- **Never write a stale entity copy.** Both price-sync loops did `updatePosition(dbPos.copy(...))` with `dbPos` read at the start of the sync; a manual close in between was silently reverted (row resurrected OPEN). Re-read by id right before writing and skip if the status changed.
- **A heuristic that marks "closed at 0 on/after expiry" as ASSIGNED matches every auto-expired row.** ImportViewModel's scan skipped EXPIRED/EXPIRED; FlexSyncWorker's copy did not → every expired short option became "הוקצה $0" on the next background sync. Expiry is not assignment evidence; explicit priced BTC/ROLLED rows are terminal; OptionEAE matches must include the expiry.
- **`updatedAt` is not an event time.** The CC-reminder staleness rule compared the snapshot with a closed CC's `updatedAt`, which every auto-expire, buy-to-close, re-import and re-scan bumps — hiding genuinely held tickers. Compare against the actual assignment settle (16:00 ET of the assignment date), the only CC outcome that removes shares.
- **IBKR `ibCommission` is SIGNED (negative = charge, positive = rebate).** Normalize by negation into the app's +expense convention; `abs()` silently turns rebates into charges.
- **Manual rows were invisible to the import by design** (`isDuplicate` skip + MANUAL refusal in `updateDuplicatePremium`). Reconciliation must be an explicit unique-match pass (IbkrReconciler) that keeps `syncSource=MANUAL` so a resync never deletes the owner's row.
- **Codex review can be unavailable for account reasons** ("usage limits for code reviews") — the Codex Gate then stays red as "not reviewed"; that is not a finding and must not be worked around by the agent (owner decides: credits or `codex-p1-acknowledged`).
- **This repo's protected-path label is `needs-owner`, not `needs-dima`** (merge-bot.yml LABEL_ESCALATE); `no-automerge` is the permanent human opt-out.
- **Wireless ADB can drop mid-task and the connect port changes**; `/proc/net/tcp` is unreadable in PRoot and a localhost connect scan (1024–65535) found no adbd port — when that happens the owner must re-enable Wireless debugging and provide the new IP:PORT (and pairing code if pairing was lost). Do not guess ports.
- **Subagents can die on the account's session rate limit** (all four investigators terminated with HTTP 429 "session limit"); plan for doing the investigation inline.

### 2026-09-07 S2 device-QA lessons (Claude Code; OPT 1ab4c82, PR #19)
- **One stored row can be claimed by SEVERAL broker cycles.** Matching each cycle independently and writing immediately lets the LAST cycle overwrite the previous one's realized P&L (BKSY: a +171.95 buy-to-close became an ASSIGNED $0). Resolve ALL claims first, give the row to the cycle whose dates agree, and insert the losers as their own rows — otherwise every re-import silently rewrites history.
- **`findDuplicate`-style helpers with `LIMIT 1` and no quantity are unsafe once a contract has two rows.** They return an arbitrary row and write the wrong cycle's values onto it, forever (PLUG/QQQ premium churned on every single import).
- **Never infer a commission sign convention from the majority of a payload.** IBKR's `ibCommission` is per-fill: negative = charge, positive = maker rebate, and BOTH appear in one export (81 of 1399 fills were credits). Verify against `fifoPnlRealized`: gross − Σ(−ibCommission) must equal IBKR's realized figure. `abs()` is always wrong; a per-payload "detected sign" is also wrong.
- **A price snapshot is not "today" when the market is closed.** Anything that renders a daily move must be gated on a trading day or it presents the last session as today's.
- **The dashboard log storm was real and is measurable**: 63k `PNL_TRACE` + 3k `PnLDebug` + 2.3k `PNL_DBG` on one load, 0 after gating. Measure the old build first so the "fixed" claim has a baseline.
- **Driving the app over ADB works well for QA**: `uiautomator dump` + regex tap (see the S2 scratch helper) reaches Settings → ייבוא מלא → סנכרן מ-IBKR → ייבא N פעולות without touching any destructive button; `adb shell am start -n com.termux/.app.TermuxActivity` hands the screen back afterwards.
- **IBKR Flex rate-limits repeat pulls** ("Too many requests have been made from this token") — wait ~90 s and retry rather than re-driving the whole flow.

### 2026-09-07 S2 review-round lessons (Claude Code; OPT 2ee5d84, PR #19)
- **A timestamp is only evidence of what actually writes it.** The CC-reminder staleness rule leaned on the snapshot's `updatedAt`, which price-only syncs bump — so it could not distinguish a held position from one that was assigned away. When you need "is this fact still true", stamp the FACT (`sharesUpdatedAt` beside `shares`), never a shared record-level timestamp.
- **When you fix a bug pattern, grep for the pattern.** The stale-copy resurrection was fixed in the worker's price loop and left untouched in the same file's partial-close block 65 lines below. Same for the ORDER/EXECUTION filter: added to the import path, missing from the worker's own `<Trade>` scrape.
- **A "read-modify-write one JSON blob" preference store is O(n²) and loses concurrent writes.** One key per id gives O(1) reads and collision-free writes for free.
- **A side table keyed by row id must record WHICH row it was learned for.** Room ids are not reused by AUTOINCREMENT, but a backup restore re-inserts rows with their original ids — without a contract check a stale record then freezes a wrong execution time and suppresses assignment detection on an unrelated position.
- **Only claim what the data says.** A worker alert line "דוח בעוד 5 ימים" is not "tomorrow"; a bare ticker regex matches ordinary English words (KEY, ALL, IT); and a price snapshot before the 04:00 ET pre-market open is still the PREVIOUS session — each one produced a confidently wrong sentence in the daily brief.
- **In a Compose Row, an unweighted wrapping label can starve its sibling to zero width** — the LTR detail carrying the ticker simply disappears at large font scales.

### 2026-09-07 S2.1 gotchas
- **Logcat is not private.** The import path was printing the raw Flex XML, the IBKR account number and the WHOLE <AccountInformation> element (legal name, home address, email, trading permissions) on every sync, plus the AlphaVantage key inside its URL. Anything with READ_LOGS, any adb logcat and any bug report could read it. Before logging a broker payload, log its SHAPE (length, offsets, presence) — never its content. FlexLogRedaction exists for the rest.
- **"unchanged" is not "correct".** The reconciler counters (updated/inserted/unchanged) describe what was WRITTEN. A row can be left unchanged and still disagree with the broker. That is why ReconciliationAudit compares against the value the app will DISPLAY (ibkrRealizedPnl ?: local) after the write, not against what was stored.
- **Grep for the precedence, not for the function.** Every bucket-(B) violation was a call to ProfitCalculator.realizedPnL(row) on a raw PositionEntity. The surfaces that were correct all went through PositionSummary, whose single constructor already applies `ibkrRealizedPnl ?:`. If a new screen touches a PositionEntity directly, it is a suspect by default.
- **All-or-nothing precedence silently drops money.** The spread-pair row used `if (either leg has ibkr) (a ?: 0.0) + (b ?: 0.0)`, so a spread with only one leg reconciled dropped the other legs entire P&L. Precedence must always be applied PER VALUE.
- **A live mark-to-market must never be titled as a result.** The Covered Put detail card said "רווח/הפסד" once the position closed while still computing from the current price. If isClosed, source the number authoritatively or do not show the card.
- **Equal quantities that SUM to the cycle mean a split, not a tie.** The three "ambiguous" contracts were one broker round trip the owner entered as several partial-close rows. Picking a winner would rewrite its numContracts to the whole cycle and stamp the whole realized P&L on it while the siblings kept theirs. Check `candidates.sumOf { numContracts } == cycle.contracts` before ever reaching for a tie-break rule.
- **observed() means a weekend can never be named as a holiday.** Fixed-date NYSE holidays shift Sat→Fri and Sun→Mon, so on Sunday 25-Dec the market-closed reason is the WEEKEND (that year the holiday is Monday the 26th). closedReason() checks isWeekend first, exactly as isNonTradingDay does.
- **A bare \bTICKER\b match is not evidence.** analyzedTickers is built that way, so "the KEY takeaway", "ALL of them" and "IT spending" became stated reasons for price moves — and passing it as one arm of an || defeated the cashtag guard entirely. A reason needs an explicit $/@ cashtag or a real company-name hit.
- **"Today" has a timezone.** The brief filtered social posts with LocalDate.now() in the DEVICE zone (Thailand, UTC+7) while every other value on the card was America/New_York, so for ~11 hours a day it mixed a Bangkok-today list into a New-York-today briefing.
- **The NYSE closes at 13:00 ET on three days a year** (day after Thanksgiving, Christmas Eve, July 3 when each is a plain weekday). A hard-coded 16:00 boundary claims regular trading for three hours after the bell. MarketCalendar.regularCloseOn(d) is the only correct source.
- **Do not claim an event lifecycle the data cannot support.** The app stores only an earnings DATE (AlertWorker caches {checked, next} and throws away Finnhub hour/epsActual), so "פורסם היום" and "צפוי בהמשך היום" are both unprovable. Expiry and price moves DO have a knowable lifecycle — they resolve at the regular close — so only those change tense.

### 2026-09-07 S2.1 (review round) gotchas
- **A partial-close slice must take only ITS OWN fills.** Filtering the Flex payload by contract alone means every earlier closing fill still inside the window is summed into each new slice, so the second partial close re-adds the first one's realized P&L. Consume fills newest-first for exactly the quantity the slice covers, and prorate one that straddles the boundary.
- **When you SPLIT a fill, split its commission too.** A zero-crossing fill belongs to two cycles; copying the whole commission into both makes realizedPnL subtract it twice. Prorate by quantity and give the rounding remainder to the half that becomes a realized number.
- **A write-only persisted value is a bug waiting for a sign change.** The tax loss carry-forward was only ever written on a loss and never cleared, so a year that later turned profitable kept deducting a stale loss forever. If you persist a derived number, write it on EVERY branch, including the zero case.
- **Never put a realized P&L on a row that stays OPEN.** applyClosed's fall-through kept the status OPEN but still stamped the cycle's realized figure and close commission. Nothing clears that field, so it silently overrode the owner's later manual close and made the ticker analysis count an open row as a finished trade.
- **A fallback matcher must never touch a live position.** Re-homing a losing cycle onto "any free row of the same contract and quantity" could pick a still-OPEN row and close it. A cycle with no closed row of its own belongs in an orphan insert.
- **Splitting a row invalidates its broker figure.** ibkrRealizedPnl describes the whole cycle, so copying it onto both halves of a partial close double-counts it in every aggregate that prefers it. Clear it and let the next sync re-reconcile. Likewise, if the owner explicitly edits a reconciled close, drop the broker value — otherwise the screen accepts input and changes nothing.
- **An audit that compares a value against itself proves nothing.** The first version compared the object applyClosed had just built against the cycle it was built from. Audit the PERSISTED row, and make the verdict account for what was REFUSED — a run that skipped cycles has not verified them, so it must not call itself CLEAN.
- **"Nothing happened" is a claim and needs evidence.** An empty movers list means either nothing moved OR no price could be computed for any ticker. Check that at least one real datum exists before asserting the negative.
- **A keyless `remember { now() }` freezes the clock for the life of the composition** — in a LazyColumn it also re-runs only when the item scrolls out and back, so the value depends on scroll history. Use produceState with a delay loop, and give a Flow that reports "now" its own ticker.
- **Removing a truncation is only half the job.** Dropping the "(+N)" suffix while a maxLines clamp remains just replaces a counted truncation with an uncounted ellipsis. And an unbounded wrapping list needs non-breaking spaces inside each item, or a ticker ends up on a different line from its own number.
