...
ActivityEventEntity.timestamp defaults to now() — expiry/assignment events MUST pass an explicit timestamp = expiration/close date @ 16:00 ET (America/New_York), else the feed shows the detection time. premiumPerContract/estimatedPremium are PER-SHARE app-wide; multiply ×100 only for per-contract DISPLAY.

Realized P&L is COMPUTED (ProfitCalculator.realizedPnL), not stored — fixing the calc retroactively fixes all closed positions; no data migration needed. CSP assignment realized = $0 option P&L (premium → cost basis, realized at stock sale) in BOTH ProfitCalculator.realizedPnL (central) and ClosePosition (preview). ibkrRealizedPnl (from Flex import) overrides realizedPnL in most call sites but several call realizedPnL directly, so the calc itself must be correct.

FlexSyncWorker SMART MERGE keeps tickers on partial Flex results; it now ALSO runs the BA3 prune (only when holdings returned) so fully-sold tickers leave the portfolio/CC-reminder/alerts. Sold-out detection = ticker not in {open/draft positions ∪ this sync's holdings} and not manual. Worker uses positionDao.getAllPositionsOnce() (no repository injected).
