...
- **Last commit:** `75722b0` (Group K prime — v8 pre/post-market-aware batch fetch for all dashboard refresh paths, removed same-price write-back skip)
- **Recent commits:** `8a44bd4` → `9ddca1c` (Group A) → `f1c3e9a` (Group B) → `16fd852` (Group C) → `fdc1cf1` (Group D prime) → `7df9465` (Group E prime) → `589b44e` (Group F prime) → `646a1e0` (Group G prime) → `be9a23e` (Group H prime) → `996ffe6` (Group I prime) → `9f79c9f` (Group J prime) → `75722b0` (Group K prime)
...
| # | Issue | Status |
|---|---|---|
| N4 | Off-market-hours: all data wrong (CC reminder, abnormal alerts, per-ticker numbers/percent) - stock prices do not update outside market hours | fixed in K prime (J' covered open-ticker union but v7 quote endpoint never returned pre-market AND a same-price write-back guard skipped every position; K' switches to v8 chart with includePrePost=true and removes the guard), awaiting device verify |
