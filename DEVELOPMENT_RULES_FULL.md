...
### תקציב shell processes ו־background jobs

יציבות המכשיר קודמת למקביליות מיותרת.

- לכל session של Claude Code או Codex מותר להחזיק לכל היותר 6 shell processes פעילים בסך הכול, כולל foreground shells, background jobs, waiters ותהליכי shell שנפתחו על ידי subagents. היעד הרגיל הוא 2–4 פעילים, לא 6.
- עד 4 subagents הוא מקסימום מותר ולא יעד שחייבים למלא. יש לפתוח רק את מספר ה־subagents שמועיל בפועל למשימה.
- אסור לצבור `sleep 600`, `sleep 900` או waiters דומים. לכל היותר waiter אחד פעיל לאותה מטרה.
- לפני פתיחת shell או background job חדש לצורך CI, review, build או המתנה אחרת, הסוכן חייב לבדוק אם כבר קיים waiter לאותה מטרה ולהשתמש בו במקום לפתוח waiter נוסף.
- polling של CI חייב להתבצע באמצעות תהליך polling יחיד שמבצע בדיקות מחזוריות, או באמצעות בדיקה חד־פעמית כאשר צריך. אסור לפתוח shell חדש בכל סבב המתנה.
- המתנה ל־GitHub CI מוגבלת ל־shell אחד פעיל לכל session, ללא קשר למספר ה־checks או ה־subagents.
- אסור להשתמש ב־background `sleep` רק כדי “להחזיק מקום” עד ש־CI, review או build יסתיימו.
- כאשר subagent סיים את עבודתו, אין להשאיר את ה־shell/background process שלו חי ללא צורך.
- אם מספר ה־shell processes הפעילים הגיע לתקרה של 6, אסור לפתוח shell חדש עד שסוגרים, מסיימים או ממתינים לסיום של תהליך קיים.
- כאשר אפשר לקבל את אותה תוצאה ללא process מתמשך, יש להעדיף בדיקה חד־פעמית על waiter/background process.
- אין לעקוף את המגבלה באמצעות subagent נוסף, subshell, wrapper או background job חדש.
...
### חיבור ADB אלחוטי — מסלול מוכח במכשיר/Termux של בעל הפרויקט

זהו המסלול שכבר עבד בפועל בסביבת Samsung + Termux של בעל הפרויקט. כאשר בעל הפרויקט מבקש "איך לחבר ADB", "לחבר את הטלפון ב־ADB", "ADB wireless" או בקשה שקולה, יש להחזיר את ההודעה המוכנה להלן ולא להמציא מסלול אחר כל עוד המסלול המוכח לא נכשל בפועל.

הודעה מוכנה:

> אם `android-tools` ו־`fakeroot` עדיין אינם מותקנים ב־Termux:
>
> ```bash
...
>
> בטלפון פתח:
> **הגדרות מפתחים → ניפוי באגים אלחוטי → שיוך מכשיר באמצעות קוד שיוך**.
>
> מומלץ להשאיר את חלון השיוך פתוח ב־split screen עם Termux.
>
> מהחלון הזה קח את `IP:PAIR_PORT`, וב־Termux הרץ:
>
> ```bash
> ANDROID_NO_USE_FWMARK_CLIENT=1 fakeroot adb pair IP:PAIR_PORT
> ```
>
> כשהפקודה מבקשת קוד, הזן את קוד השיוך בן 6 הספרות שמופיע בטלפון.
>
> צריך לקבל:
>
> ```text
...
> ```
>
> עכשיו חזור למסך הראשי של **ניפוי באגים אלחוטי**. אל תשתמש שוב בפורט של ה־pairing. קח משם את ה־`IP:CONNECT_PORT` הראשי שמוצג עבור החיבור, והריץ:
>
> ```bash
> ANDROID_NO_USE_FWMARK_CLIENT=1 fakeroot adb connect IP:CONNECT_PORT
> ```
>
> לאחר מכן אמת:
>
> ```bash
> ANDROID_NO_USE_FWMARK_CLIENT=1 fakeroot adb devices -l
> ```
>
> החיבור תקין כאשר המכשיר מופיע במצב:
>
> ```text
> device
> ```
>
> אם צריך שאבנה את הפקודות עם הערכים המדויקים, שלח קודם את `IP:PAIR_PORT` ואת קוד 6 הספרות מחלון **"שיוך מכשיר באמצעות קוד שיוך"**. אחרי ה־pairing שלח את `IP:CONNECT_PORT` מהמסך הראשי של **ניפוי באגים אלחוטי**.

כללי שימוש במסלול:

- `PAIR_PORT` ו־`CONNECT_PORT` אינם אותו פורט בהכרח; יש לקחת כל אחד מהמסך המתאים.
- גם אם pairing בוצע בעבר, אם `adb connect` אינו מצליח ניתן לבצע pairing מחדש במסלול המוכח.
- אין להתחיל ב־`adb mdns`, `adb kill-server` או מסלולי troubleshooting אחרים כאשר המשתמש רק מבקש את דרך החיבור הרגילה. משתמשים בהם רק אם המסלול המוכח נכשל בפועל ויש ראיה שצריך troubleshooting נוסף.
- אין לבקש מהמשתמש לנחש איפה הערכים נמצאים; יש לומר במפורש מאיזה מסך לקחת כל IP/port וקוד.
- כאשר המשתמש מספק את הערכים, יש להחזיר את הפקודה המלאה עם הערכים במקום placeholders, בלי להוסיף שלבים שלא נדרשו.

#### תקלה מוכרת: `error: protocol fault (couldn't read status): Success`

אם לאחר `adb pair` והזנת קוד השיוך מתקבלת הודעה מהצורה:
...
אין לפרש את המילה `Success` כהצלחת pairing. זהו נוסח שגיאה מטעה של ADB כאשר תהליך ה־pairing נכשל/לא החזיר handshake תקין.

יש לפעול בסדר הבא, בלי לדלג אוטומטית למסלולי troubleshooting אחרים:

1. אם המכשיר כבר מופיע תחת **Paired devices** מזיווג קודם, אפשר קודם לנסות את שלב `adb connect` עם `IP:CONNECT_PORT` מהמסך הראשי של **ניפוי באגים אלחוטי**. pairing אמור להידרש רק פעם אחת כל עוד ההרשאה לא נשכחה/בוטלה.

2. אם נדרש pairing חדש, יש לוודא שהפקודה משתמשת ב־`IP:PAIR_PORT` מתוך חלון **"שיוך מכשיר באמצעות קוד שיוך"**, ולא ב־`CONNECT_PORT` מהמסך הראשי.

3. יש לסגור את חלון ה־pairing הישן, לפתוח אותו מחדש ולקבל `PAIR_PORT` וקוד 6 ספרות חדשים. יש להשאיר את החלון פתוח והמסך ער בזמן ההזנה, רצוי ב־split screen עם Termux.

4. יש להשתמש במסלול האינטראקטיבי שכבר עבד בפועל: `adb pair IP:PAIR_PORT`, ולאפשר ל־ADB עצמו לבקש את קוד 6 הספרות. אין להעביר את הקוד כארגומנט כברירת מחדל.

5. אם אותה שגיאה חוזרת, יש לסגור את חלון השיוך הנוכחי, לעבור ל־Termux host, לאפס את daemon של ADB ורק לאחר מכן ליצור חלון pairing חדש:

```bash
ANDROID_NO_USE_FWMARK_CLIENT=1 fakeroot adb kill-server
```

במכשיר/Termux של בעל הפרויקט הפעולה הזאת כבר פתרה בפועל את שגיאת `protocol fault`, ולכן כאשר השגיאה הזאת חוזרת יש להשתמש ב־`kill-server` כשלב התיקון המוכח לפני מעבר ל־troubleshooting נוסף.

חשוב: `kill-server` מתבצע **לפני** קבלת `PAIR_PORT` וקוד חדשים. לאחריו יש לכבות ולהדליק מחדש **ניפוי באגים אלחוטי** אם נדרש, לפתוח שוב **"שיוך מכשיר באמצעות קוד שיוך"**, לקבל `PAIR_PORT` וקוד חדשים, ואז לבצע מיד `adb pair`. אין להריץ `kill-server` בין קבלת הערכים החדשים לבין `adb pair`.

6. אם התקלה עדיין חוזרת, יש לבדוק:
   - שאין VPN פעיל שמפריע לנתיב המקומי;
   - ש־Wireless debugging עדיין מאופשר;
   - ש־`android-tools` מעודכן ב־Termux;
   - שלא נעשה שימוש בפורט ישן מחלון pairing קודם.

7. בסביבת same-device Termux, ורק אם המסלול המוכח עם ה־IP שמוצג נכשל שוב עם ערכים טריים, ניתן לנסות fallback עם `localhost` ואותו `PAIR_PORT` הטרי:

```bash
ANDROID_NO_USE_FWMARK_CLIENT=1 fakeroot adb pair localhost:PAIR_PORT PAIR_CODE
```

אין לדווח pairing כמוצלח עד שמתקבל במפורש:

```text
Successfully paired to ...
```

לאחר pairing מוצלח, יש לחזור למסלול הקנוני: לקחת `IP:CONNECT_PORT` מהמסך הראשי של **ניפוי באגים אלחוטי**, להריץ `adb connect`, ואז לאמת `adb devices -l` עם מצב `device`.

### בידוד ADB מ־PRoot / Claude / Codex

נמצא ואומת בפועל כשל שבו ADB server שנוצר מתוך Claude Code תחת PRoot נשאר במצב `tracing stop` עם `TracerPid` של ה־PRoot; גם `SIGKILL` לתהליך ה־ADB לא פינה אותו כל עוד ה־PRoot tracer נשאר פעיל. סגירת סשן Claude/PRoot שחררה את התהליכים ולאחר מכן חיבור ADB מ־Termux הצליח.

לכן חלים כללי בטיחות קבועים:

- ADB server lifecycle ו־Wireless Debugging pairing הם פעולות **Termux-host-only**. אין להפעיל מתוך Debian/PRoot, Codex או Claude Code: `adb start-server`, `adb kill-server`, `adb server nodaemon`, pairing troubleshooting שמקים server חדש, או kill/pkill של ADB server.
- `cauto` ו־`clauto` חייבים להעביר לסביבת Debian את `ADB_SERVER_SOCKET=tcp:127.0.0.1:5037`. כך ADB clients בתוך PRoot פונים ל־ADB server של Termux כ־remote server. אם השרת אינו זמין, הם חייבים להיכשל ולא להקים server חדש מתוך PRoot.
- לפני שימוש ADB מתוך agent יש לוודא שהחיבור/שרת כבר הוקם בצד Termux. אם אינו זמין, הסוכן חייב לבקש מבעל הפרויקט לבצע את מסלול החיבור הקנוני ב־Termux; אין לנסות “לתקן” זאת באמצעות הקמת ADB server מתוך PRoot.
- אין לבצע סריקת טווחי פורטים כדי לגלות `PAIR_PORT` או `CONNECT_PORT`. הערכים נלקחים ישירות ממסכי Wireless debugging. בפרט אסור לפתוח מאות threads/workers לסריקת אלפי פורטים.
- אם תהליך ADB מופיע עם `TracerPid` שאינו `0` ושייך ל־PRoot, אין להמשיך pairing או לפתוח עוד ADB processes. יש לסיים באופן מבוקר את סשן ה־PRoot שיצר אותו, לאמת שהתהליך נעלם, ורק אז לחבר מחדש מ־Termux.
- פעולות device רגילות לאחר שה־Termux-host ADB server כבר פעיל, כגון `adb devices`, `adb install`, `adb shell`, `adb push` ו־E2E, רשאיות להתבצע מה־agent רק דרך ה־host ADB server שהוגדר לעיל; אסור להקים server חלופי מתוך PRoot.
- אין לטעון שהבידוד תקין עד שנבדקו בפועל: host ADB עובד בזמן Claude/Codex פתוחים; process ה־ADB server מציג `TracerPid: 0`; וסגירת/פתיחת agent sessions אינה שוברת את חיבור ה־ADB.

### helper קבוע בשם `adb` ב־Termux

בסביבת Termux של בעל הפרויקט, הפקודה `adb` ללא arguments מיועדת להפעיל helper אינטראקטיבי לחיבור ADB אלחוטי. ה־helper צריך:

- להשאיר `adb <args...>` תואם להתנהגות הרגילה של binary ה־ADB, כדי שסקריפטים ופקודות קיימים כגון `adb devices` לא יישברו.
- להשתמש כברירת מחדל בקידומת ה־LAN שנבדקה בפועל: `192.168.1.`.
- לבצע את הזרימה באופן סדרתי בהתאם למסכי Android, ולא לבקש `CONNECT_PORT` לפני שה־pairing הסתיים בהצלחה.
- בשלב הראשון לבקש רק את הספרות האחרונות של כתובת ה־IP ואת `PAIR_PORT`. לאחר מכן ה־helper מריץ `adb pair` ב־foreground, ו־ADB עצמו מבקש מהמשתמש את קוד pairing בן 6 הספרות.
- להריץ `ANDROID_NO_USE_FWMARK_CLIENT=1` ו־`fakeroot` באופן אוטומטי.
- לבצע `adb kill-server` בתחילת האשף, **לפני** שהמשתמש פותח/קורא את חלון **"שיוך מכשיר באמצעות קוד שיוך"** ולפני איסוף `PAIR_PORT` וקוד השיוך. שלב ה־reset חייב להיות time-bounded כדי שלא ייתקע ללא הגבלה. אין להריץ `kill-server` לאחר שכבר נאספו `PAIR_PORT` וקוד טריים, כדי שלא להפריד בין קבלת ערכי השיוך לבין ניסיון ה־pairing עצמו.
- לאחר איסוף הספרות האחרונות של ה־IP ו־`PAIR_PORT`, יש להריץ את פקודת ה־pairing המוכחת בדיוק בצורתה האינטראקטיבית: `ANDROID_NO_USE_FWMARK_CLIENT=1 fakeroot adb pair IP:PAIR_PORT`. אין להעביר את קוד השיוך כארגומנט לפקודה ואין ללכוד/להסתיר את stdout/stderr של `adb pair`; ADB עצמו חייב לבקש את קוד 6 הספרות ב־foreground והמשתמש מזין אותו ישירות לפרומפט של ADB.
- רק לאחר pairing מוצלח, להורות למשתמש לחזור למסך הראשי של **ניפוי באגים אלחוטי**; חלון **"שיוך מכשיר באמצעות קוד שיוך"** אמור להיסגר לאחר pairing מוצלח, ואם הוא נשאר פתוח יש לסגור אותו. ה־helper חייב להמתין לאישור המשתמש שהוא רואה את המסך הראשי.
- רק אז לבקש `CONNECT_PORT` מהמסך הראשי, לבצע `adb connect`, ולבסוף `adb devices -l`.
- לעצור ולא לדווח הצלחה אם pairing או connect נכשלו.
- אם קידומת ה־IP משתנה בעתיד, אין להניח ש־`192.168.1.` עדיין נכונה; יש לעדכן את helper לפי כתובת ה־IP שמוצגת בפועל ב־Wireless debugging.
