...
4. אפשר להעביר את הקוד ישירות באותה פקודה כדי לצמצם סיכון לשימוש בקוד/פורט שכבר התחלפו:

```bash
ANDROID_NO_USE_FWMARK_CLIENT=1 fakeroot adb pair IP:PAIR_PORT PAIR_CODE
```

5. אם אותה שגיאה חוזרת עם `PAIR_PORT` וקוד טריים, יש לאפס את daemon של ADB ואז ליצור pairing חדש:
...
לאחר מכן יש לכבות ולהדליק מחדש **ניפוי באגים אלחוטי**, לפתוח שוב **"שיוך מכשיר באמצעות קוד שיוך"**, ולקבל שוב `PAIR_PORT` וקוד חדשים לפני ניסיון נוסף. `adb pair` יפעיל את ה־daemon מחדש לפי הצורך.
...
- לבקש מהמשתמש רק ערכים מספריים: הספרות האחרונות של כתובת ה־IP, `PAIR_PORT`, קוד pairing בן 6 ספרות ו־`CONNECT_PORT`.
- להריץ `ANDROID_NO_USE_FWMARK_CLIENT=1` ו־`fakeroot` באופן אוטומטי.
- לבצע `adb kill-server` לפני pairing חדש, משום שזה כבר פתר בפועל את שגיאת `protocol fault` בסביבה הזאת.
- לבצע `adb pair`, לוודא שהתקבל `Successfully paired to ...`, לאחר מכן לבצע `adb connect`, ולבסוף `adb devices -l`.
